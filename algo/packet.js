...
class PacketProcessor {
    ...
    _processAoiSyncDelta(aoiSyncDelta) {
        ...
        const BuffInfoSync = aoiSyncDelta.BuffInfos;
        if (BuffInfoSync && BuffInfoSync.BuffInfos) {
            const BuffInfos = BuffInfoSync.BuffInfos;
            for (const BuffInfo of BuffInfos) {
                try {
                    if (this.userDataManager.buffMonitor) {
                        ...
                        const entityUid = targetUuid.toNumber();
    ...
    _processSyncContainerData(payloadBuffer) {
        ...
        try {
            ...
            if (vData.BuffInfo && vData.BuffInfo.AllBuffDbData && this.userDataManager.buffMonitor) {
                ...
                this.logger.debug(`BuffInfo found but AllBuffDbData is empty or buffMonitor not ready`);
            }
